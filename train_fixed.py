...
from dataset_reader_fixed import SyntheticGridCellsDataset, RatMotionModel
import scores
...
# Store
parser.add_argument('--saver_results_directory', type=str, default='./results_sge',
...
def visualize_figure1b(model, device, save_dir, epoch, env_size):
    ...
    # 1. 生成数据 (现在包含正确的 init_pos)
    traj = motion_model.generate_trajectory(750) 
    
    ego_vel = torch.from_numpy(traj['ego_vel']).float().unsqueeze(0).to(device)
    # 使用 traj['init_pos'] 而不是 target_pos[0]
    init_pos = torch.from_numpy(traj['init_pos']).float().unsqueeze(0).to(device)
    init_hd = torch.from_numpy(traj['init_hd']).float().unsqueeze(0).to(device)
    
    # ... (中间初始化 Ensembles 的代码保持不变) ...
    init_conds = []
    ...
    # Ground Truth 还是 target_pos (P1...PT)
