# scores.py
# Reproduced and updated for Python 3
...
import math
import matplotlib
# matplotlib.use('Agg') # Controlled by utils/train
import matplotlib.pyplot as plt
import numpy as np
import scipy.signal
import scipy.stats # Added for binned_statistic_2d
...
class GridScorer(object):
  ...
  def plot_sac(self, sac, mask_params=None, ax=None, title=None, *args, **kwargs):
    ...
    if mask_params is not None:
      ...
      ax.add_artist(
          plt.Circle((center, center), mask_params[0] * self._nbins,
                     fill=False, edgecolor='k'))
      ax.add_artist(
          plt.Circle((center, center), mask_params[1] * self._nbins,
                     fill=False, edgecolor='k'))
    ax.axis('off')
    if title is not None:
      ax.set_title(title)